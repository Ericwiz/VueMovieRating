...
<template>
  <div
    class="w-full bg-movie_bg-02 h-fit bg-cover bg-center bg-pink-950 bg-blend-multiply px-4 sm:px-8 md:px-16 py-16 2xl:pt-20"
  >
    ...
    <div
      v-if="movies"
      class="grid sm:grid-cols-2 lg:grid-cols-4 gap-14 cursor-pointer items-center"
    >
      <div
        v-for="(movie, index) in sampleMovies"
        :key="movie.imdbID"
        :class="{ 'hidden lg:block': index > 1, 'hidden sm:block': index === 1 }"
        class="justify-self-center xs:justify-self-auto"
      >
        <RouterLink :to="{ name: 'movie.details', params: { id: movie.imdbID } }">
          <img
            :src="movie.Poster"
            alt="movie poster"
            class="h-fit w-fit xs:w-full sm:h-96 w-84 rounded-sm 2xl:h-full"
          />
          <div class="py-3">
