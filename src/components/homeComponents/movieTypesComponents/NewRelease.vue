...
onBeforeMount(() => {
  fetchMovies('love', '2023').then((response) => {
...
<template>
  <div v-if="latestMovies" class="grid gap-8 grid-cols-1 xs:grid-cols-2 text-white 2xl:gap-16">
    <div v-for="movie in latestMovies" :key="movie.imdbID">
      <RouterLink
        class="flex flex-col lg:flex-row space-y-3 md:space-x-6"
        :to="{ name: 'movie.details', params: { id: movie.imdbID } }"
      >
        <img
          :src="movie.Poster"
          alt="poster"
          class="rounded-sm w-full h-full xs:h-72 lg:w-60 2xl:w-2/4 2xl:h-full"
        />
        <div class="flex flex-col space-y-2 2xl:space-y-4">
          ...
          <p class="text-[0.8rem] lg:text-[0.9rem] hidden lg:block text-gray-300 2xl:text-lg">
            {{ movie.Plot }}
          </p>
        </div>
      </RouterLink>
